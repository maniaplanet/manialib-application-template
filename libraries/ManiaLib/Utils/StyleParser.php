...
class StyleParser
{
	 ...
	 */
	static function declareFont($name, $normal, $bold=null, $italic=null,
		$boldItalic=null)
	{
		$this->fonts[$name] = array(
			...
			StyleParser::BOLD | StyleParser::ITALIC => $boldItalic ? : ($bold ? : ($italic ?
							: $normal))
		);
	}
	 ...
	 */
	static function onImage($string, $image, $fontName, $x=0, $y=0, $size=10,
		$defaultColor='000')
	{
		if($size <= 5)
			self::onImageQuality($string, $image, $fontName, $x, $y, $size,
				$defaultColor, 3);
		else if($size <= 10)
			self::onImageQuality($string, $image, $fontName, $x, $y, $size,
				$defaultColor, 2);
	 ...
	 * @param string $defaultColor Default text color in 3 or 6 hexadecimal characters
	 */
	static function onImageFast($string, $image, $fontName, $x=0, $y=0, $size=10,
		$defaultColor='000')
	{
		...
		foreach(self::parseString($string) as $token)
			$xOffset += $token->onImage($image, $fontName, $x + $xOffset, $y, $size,
				$defaultColor, 1);
	}
	 ...
	 */
	static function onImageQuality($string, $image, $fontName, $x=0, $y=0,
		$size=10, $defaultColor='000', $precision=2)
	{
		$defaultColor = Color::StringToRgb24($defaultColor);
		...
		$hugeBrush = imagecreatetruecolor(imagesx($brush) * $factor,
			imagesy($brush) * $factor);
		imagefill($hugeBrush, 0, 0, 0x7fffffff);

		$xOffset = 0;
		foreach($tokens as $token)
			$xOffset += $token->onImage($hugeBrush, $fontName, $xOffset,
				imagesy($hugeBrush) * .75, $factor * $size, $defaultColor, $factor);

		$brushX = $x + imagesx($brush) / 2;
		$brushY = $y - imagesy($brush) * .25;
		imagecopyresampled($brush, $hugeBrush, 0, 0, 0, 0, imagesx($brush),
			imagesy($brush), imagesx($hugeBrush), imagesy($hugeBrush));
	...
	{
		$rawTokens = preg_split('/(\$(?:[0-9a-f][^\$]{0,2}|[lhp](?:\[.*?\])?|.))/iu',
...
abstract class Color
{
	...
	static function Contrast($colorRgb, $backgroundRgb,
		$contrast=StyleParser::CONTRAST_AUTO)
	{
...
class TextToken
{
		...
		if($this->style)
		{
			...
			if($this->style & StyleParser::COLORED)
			{
				if(StyleParser::getBackground() !== null && StyleParser::getContrast() !== null)
				{
					...
					$color = Color::Contrast($color, StyleParser::getBackground(),
							StyleParser::getContrast());
			...
			return '<span style="'.$styles.'">'.htmlentities($this->text, ENT_QUOTES,
					'UTF-8').'</span>';
		}
		else
	...
	{
		$fontFile = StyleParser::getFont($fontName,
				$this->style & (StyleParser::BOLD | StyleParser::ITALIC));
		...
		if($this->style & StyleParser::WIDE || $this->style & StyleParser::NARROW)
		{
			...
			foreach((array) $this->text as $char)
			{
				...
				$temp = imagecreatetruecolor(($bBox[2] - $bBox[0]) * 2,
					($bBox[3] - $bBox[5]) * 2);
				...
				if($this->style & StyleParser::SHADOWED)
					imagettftext($temp, $size, 0, -$bBox[0] + $shadowOffset,
						$bBox[3] - $bBox[5] + $shadowOffset, 0x3f000000, $fontFile, $char);
				imagettftext($temp, $size, 0, -$bBox[0], $bBox[3] - $bBox[5], $color,
					$fontFile, $char);
				imagecopyresampled($brush, $temp, 0, 0, 0, 0, imagesx($brush),
					imagesy($brush), imagesx($temp), imagesy($temp));
				imagesetbrush($image, $brush);
				imageline($image, $brushX, $brushY, $brushX, $brushY, IMG_COLOR_BRUSHED);
		...
		else
		{
			foreach((array) $this->text as $char)
			{
				...
				if($this->style & StyleParser::SHADOWED)
					imagettftext($image, $size, 0, $x + $width - $bBox[0] + $shadowOffset,
						$y + $shadowOffset, 0x3f000000, $fontFile, $char);
				imagettftext($image, $size, 0, $x + $width - $bBox[0], $y, $color,
					$fontFile, $char);
